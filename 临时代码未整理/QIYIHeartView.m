...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        CGFloat a = (random()%101)*1.0/100.0;
        _fillColor = [UIColor colorWithRed:(r)/255.0 green:(g)/255.0 blue:(b)/255.0 alpha:a];
...
- (void)animateHearFlyInView:(UIView *)view
{
    ...
    [UIView animateWithDuration:totalAnimationDuration animations:^{
        self.transform = CGAffineTransformMakeRotation(rotationDirection * M_PI/(6 + rotationFraction * 0.2));
    } completion:NULL];
    ...
    CGPoint endPoint = CGPointMake(heartCenterX + rotationDirection * 2 * heartWidth, viewHeight / 5.0 + arc4random_uniform(viewHeight/4.0));   //以至少高度的五分之一为起点加上 0到四分之一高度之间的随机数
    //生成控制点 也是在一定范围内随机
